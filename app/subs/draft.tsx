    ...
    const fetchGoogleUserData = async () => {
      ...
      if (session) {
        try {
            ...
            const response_data = await axios.get(`${api_url}/get-google-user-profile/${session?.user?.email}`);
            ...
            const response_data_user = await axios.get(`${api_url}/subscribed-properties/google/${response_data.data.user}/`);

        
            console.log("Related info: ", response_data_user.data);

            const { google_user_properties, user_properties } =  response_data_user.data;
                ...
                // const response_sub = await axios.post('${api_url}/subscribed-properties/', {
      ...
      if (token) {
        try {
            ...
            const response_data_user = await axios.get(`${api_url}/subscribed-properties/regular/${response_data.data.user}/`);
            const { google_user_properties, user_properties } =  response_data_user.data;
    ...
    <div className="flex h-screen">
      ...
      <div className={`flex-1 p-10 text-gray-700 overflow-y-auto ${isSidebarOpen ? '' : 'w-full'} md:ml-64`}>
        ...
        <div className="flex justify-between items-center mb-6">
          <h2 className="text-3xl font-semibold">Les publications de mes abonnes</h2>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {Array.isArray(properties) && properties.length > 0 ? (
            properties.map((property) => (
              <PropertyCard key={(property as { id: string }).id} property={property} />
            ))
          ) : (
